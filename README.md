...
ADMINS = {6603524612, 7773526534, 8157411319, 7487899746}
...
def build_compact_message(chat_id):
    ...
    parts = []
    parts.append(f"今日入款 ({inc_count})")
    parts.extend(income_lines)
    parts.append("")
    parts.append(f"今日下发 ({pay_count})")
    parts.extend(payout_lines)
    parts.append("")
    parts.append(f"总入款 : {fmt_inr_plain(total_income_inr)} | {fmt_usd(total_income_usd)}")
    parts.append(f"汇率 : {rate}")
    parts.append(f"交易费率 : {fee}%")
    parts.append("")
    parts.append(f"已下发 : {fmt_inr_plain(total_payout_inr)} | {fmt_usd(total_payout_usd)}")
    parts.append(f"应下发 : {fmt_inr_plain(total_income_inr)} | {fmt_usd(total_income_usd)}")
    parts.append(f"未下发 : {fmt_inr_plain(not_yet_inr)} | {fmt_usd(not_yet_usd)}")
    parts.append("")
    return "\n".join(parts)
